...
from utils.utils import (DataProcessor, InputExample,
                         read_column_tagged_file)
from utils.generic import pad_sentences
from utils.io import (validate_existing_directory,
                                    validate_existing_filepath)
from utils.text import (character_vector_generator,
...
class TokenClsInputExample(InputExample):
    ...
    def __init__(self, guid: str, text: str, tokens: List[str], feats: List[list], label: List[str]=None):
        ...
        """
        super(TokenClsInputExample, self).__init__(guid, text, feats, label)
        self.tokens = tokens
...
class TokenClsProcessor(DataProcessor):
    ...
    def _create_examples(lines, set_type):
        ...
        for i, line in enumerate(lines):
            if len(line) == 2:
                sentence, feats, labels = line[0], None, line[1]
            else:
                sentence, feats, labels = line[0], line[1], line[2]
            guid = "%s-%s" % (set_type, i)
            text = ' '.join(sentence)
            examples.append(TokenClsInputExample(guid=guid, text=text, 
                tokens=sentence, feats=feats, label=labels))
        return examples
