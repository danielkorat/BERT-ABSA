...
from utils.core_nlp_doc import CoreNLPDoc
from submodules.absapp import INFERENCE_OUT
from submodules.absapp.inference.data_types import Term, TermType, Polarity, SentimentDoc,\
    SentimentSentence, LexiconElement
from submodules.absapp.utils import _read_lexicon_from_csv, load_opinion_lex, \
...
class SentimentInference(object):
    ...
    def run(self, doc: str = None, parsed_doc: CoreNLPDoc = None) -> SentimentDoc:
        ...
        for sentence in parsed_doc.sentences:
            ...
            if events:
                senti_sent = SentimentSentence(sentence[0]['start'], sentence[-1]['start'] + sentence[-1]['len'] - 1, events)
                sentiment_doc.sentences.append(senti_sent)
            sentiment_doc.tokens.append([(tok['start'], tok['len'], tok['text']) for tok in sentence])
    ...
    def _extract_event(self, aspect_row: LexiconElement, parsed_sentence: list) -> tuple:
        ...
        real_aspect_indices = _consolidate_aspects(aspect_row.term[:4], parsed_sentence)
    ...
    def _modify_for_multiple_word(cur_tkn, parsed_sentence, index_range):
        ...
        if len(index_range) >= 2:
            start = parsed_sentence[index_range[0]]["start"]
            length = len(parsed_sentence[index_range[0]]["text"])
            for i in index_range[1:]:
                length = int(length) + len(parsed_sentence[i]["text"]) + 1
        return start, length
    ...
    def _detect_opinion_aspect_events(self, aspect_index, parsed_sent, aspect_key, index_range):
        ...
        for tok_i, tok in enumerate(parsed_sent):
            ...
            # if aspect_tok found
            for (asp_start, asp_len), opinion in aspect_op_pair:
