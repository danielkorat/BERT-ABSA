...
from submodules.absapp import INFERENCE_LEXICONS
from submodules.absapp.inference.data_types import LexiconElement, Polarity
from submodules.absapp.train.data_types import OpinionTerm
from utils.spacy_bist import SpacyBISTParser
from utils.io import download_unlicensed_file
...
def parse_txt(parser: SpacyBISTParser, txt_path: Union[str, PathLike],
    ...
    with open(txt_path, encoding='utf-8') as f:
        for i, doc_text in tqdm(enumerate(f)):
...
def parse_dir(parser, input_dir: Union[str, PathLike], out_dir: Union[str, PathLike] = None,
    ...
    Yields:
        CoreNLPDoc: the annotated document.
    """
    for i, (filename, file_contents) in tqdm(enumerate(_walk_directory(input_dir))):
...
def _write_aspect_lex(parsed_data: Union[str, PathLike], generated_aspect_lex: dict, out_dir: PathLike):
